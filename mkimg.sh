...
RPI_BOOT_OUT=${ANDROID_PRODUCT_OUT}/rpiboot
...
# Format as FAT32
mkfs.fat -F 32 -n "boot" "$ANDROID_PRODUCT_OUT"/boot.img

# Copy files into FAT32 image
mcopy -s -i "$ANDROID_PRODUCT_OUT"/boot.img "$RPI_BOOT_OUT"/* ::
...
rm -rf ${ANDROID_PRODUCT_OUT}/${IMGNAME}
